...
class TransactionsDateFilterStateUseCaseMockkTest {
    ...
    private val expectedFilter = SavedDateFilter(
        presetIndex = 2,
    ...
    @Test
    fun invoke_shouldReturnSuccessFilter_whenRepositoryEmitsDateFilterState() = runTest {
    ...
    @Test
    fun invoke_shouldReturnFailure_whenRepositoryFlowThrowsException() = runTest {
        val expectedError = IllegalStateException("read-failed")
        every { settingsRepository.getTransactionsDateFilterState() } returns flow {
            throw expectedError
        }
        val useCase = GetTransactionsDateFilterStateUseCase(settingsRepository)
    ...
    @Test
    fun invoke_shouldPersistFilter_whenRepositoryAcceptsDateFilterState() = runTest {
        coEvery { settingsRepository.setTransactionsDateFilterState(expectedFilter) } returns Unit
        val useCase = SetTransactionsDateFilterStateUseCase(
    ...
    @Test
    fun invoke_shouldReturnFailure_whenRepositoryRejectsDateFilterState() = runTest {
