...
class TransactionMaintenanceUseCasesMockkTest {
    ...
    @Test
    fun invoke_shouldReturnSuccess_whenUpdateTransactionSucceeds() =
        runTest(dispatcher) {
    ...
    @Test
    fun invoke_shouldReturnFailure_whenUpdateTransactionThrows() =
        runTest(dispatcher) {
            val transaction = sampleTransaction()
    ...
    @Test
    fun invoke_shouldEmitRepositoryTransactions_whenDateRangeIsProvided() =
        runTest(dispatcher) {
            ...
            val result =
                useCase(DateRange(from = from, to = to))
                    .take(1)
                    .toList()
                    .single()
                    .getOrThrow()
    ...
    @Test
    fun invoke_shouldRenameCategoryOnTransactions_whenCategoryNameChanges() =
        runTest(dispatcher) {
    ...
    @Test
    fun invoke_shouldReturnFailure_whenTransactionRepositoryThrows() =
        runTest(dispatcher) {
    ...
    fun invoke_shouldInsertOccurrencesAndUpdateTemplate_whenRecurringTransactionIsPastDue() =
        runTest(dispatcher) {
            ...
            every { transactionRepository.getRecurringTransactions() } returns
                flowOf(
                    listOf(
                        recurringTemplate,
                    ),
            ...
            coVerify(exactly = 1) {
                transactionRepository.updateTransactions(
                    match { list ->
                        list.size == 1 &&
                            list[0].id == recurringTemplate.id &&
