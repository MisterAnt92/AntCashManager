...
public object ReceiptTextParser {
    ...
    private val VAT_AMOUNT_PATTERN =
        Regex(
     ...
     */
    private val MEAL_VOUCHER_PATTERN =
        Regex(
     ...
     */
    private val ELECTRONIC_PATTERN =
        Regex(
    ...
    private val ES_CASH_PATTERN =
        Regex(
            """(?:EFECTIVO|DINERO|PAGO\s+EN\s+EFECTIVO)""",
            RegexOption.IGNORE_CASE,
        )

    private val ES_MEAL_VOUCHER_PATTERN =
        Regex(
            """(?:CHEQUE\s+RESTAURANTE|TICKET\s+COMIDA|VALE\s+COMIDA)""",
            RegexOption.IGNORE_CASE,
        )

    private val ES_ELECTRONIC_PATTERN =
        Regex(
    ...
    private val DE_CASH_PATTERN =
        Regex(
            """(?:BARGELD|BAR|BARZAHLUNG)""",
            RegexOption.IGNORE_CASE,
        )

    private val DE_MEAL_VOUCHER_PATTERN =
        Regex(
            """(?:ESSENSMARKEN|VERPFLEGUNGSGUTSCHEIN|LUNCHEON\s+GUTSCHEIN)""",
            RegexOption.IGNORE_CASE,
        )

    private val DE_ELECTRONIC_PATTERN =
        Regex(
    ...
    private val FR_CASH_PATTERN =
        Regex(
            """(?:ESPÈCES|ESPECES|PAIEMENT\s+EN\s+ESPÈCES)""",
            RegexOption.IGNORE_CASE,
        )

    private val FR_MEAL_VOUCHER_PATTERN =
        Regex(
            """(?:TICKET\s+RESTAURANT|TICKET\s+REPAS|VALE\s+REFEICAO)""",
            RegexOption.IGNORE_CASE,
        )

    private val FR_ELECTRONIC_PATTERN =
        Regex(
    ...
    public fun parse(
        ...
        val vatAmount =
            if (totalAmount > 0.0 && rawVatAmount > totalAmount * 1.01) {
     ...
     * @param language Codice lingua (it, en, es, de, fr). Default: it
     */
    public fun detectPaymentType(
        text: String,
        language: String = "it",
    ): PaymentType {
        val (mealPattern, cashPattern, electronicPattern) =
            when (language) {
