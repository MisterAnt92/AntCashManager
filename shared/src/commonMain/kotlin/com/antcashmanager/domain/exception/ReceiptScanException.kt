...
sealed class ReceiptScanException(message: String) : Exception(message) {
    ...
    data class OcrFailed(val rootCause: Throwable) : ReceiptScanException("OCR processing failed: ${rootCause.message}")

    /** Nessuna categoria di spesa disponibile per creare la transazione. */
    object NoCategoryAvailable : ReceiptScanException("No expense category available to create transaction")
}
