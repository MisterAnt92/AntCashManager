...
class AddTransactionIntegrationTest : BaseUnitTest() {
    ...
    @Test
    fun `complete flow for new expense transaction`() =
        runUnitTest {
    ...
    @Test
    fun `complete flow for new income transaction`() =
        runUnitTest {
            viewModel = createViewModel()
            testDispatcher.scheduler.advanceUntilIdle()
    ...
    @Test
    fun `complete flow for editing existing transaction`() =
        runUnitTest {
    ...
    @Test
    fun `validation prevents saving incomplete transaction`() =
        runUnitTest {
            viewModel = createViewModel()
            testDispatcher.scheduler.advanceUntilIdle()
    ...
    @Ignore("Skipped due to Android Bundle not mocked. Requires Robolectric setup")
    @Test
    fun `validation prevents saving with invalid amount`() =
        runUnitTest {
            viewModel = createViewModel()
            testDispatcher.scheduler.advanceUntilIdle()
    ...
    @Test
    fun `can create multiple transactions sequentially`() =
        runUnitTest {
            // Transaction 1
            viewModel = createViewModel()
            testDispatcher.scheduler.advanceUntilIdle()
    ...
    @Test
    fun `handles recurring transaction setup`() =
        runUnitTest {
            viewModel = createViewModel()
            testDispatcher.scheduler.advanceUntilIdle()
