...
sealed class SettingsDataEvent {
    // ── Data Encryption ──
    data class SetDataEncryptionEnabled(
        val enabled: Boolean,
    ) : SettingsDataEvent()

    // ── Suggestions ──
    data class SetSuggestionsEnabled(
        val enabled: Boolean,
    ) : SettingsDataEvent()
    ...
    data class OnAutoBackupFolderSelected(
    ...
    data class OnBackupFileSaveError(
        val message: String,
    ) : SettingsDataEvent()
    ...
    data class OnRestoreFileReadError(
        val message: String,
    ) : SettingsDataEvent()
