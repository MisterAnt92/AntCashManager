...
sealed class ReceiptScanEvent {
    data class ScanReceipt(val imageBytes: ByteArray) : ReceiptScanEvent()
    data class UpdateTitle(val title: String) : ReceiptScanEvent()
    data class UpdatePayee(val payee: String) : ReceiptScanEvent()
    data class UpdateLocation(val location: String) : ReceiptScanEvent()
    data class UpdateNotes(val notes: String) : ReceiptScanEvent()
    data class UpdateAmount(val amount: Double) : ReceiptScanEvent()
    data class SelectCategory(val category: Category) : ReceiptScanEvent()
    data class SelectPaymentType(val paymentType: PaymentType) : ReceiptScanEvent()
